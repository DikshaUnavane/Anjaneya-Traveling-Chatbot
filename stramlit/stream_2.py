...
def get_chat_response(user_message):
    ...
    user_message = ("hello", "hi", "hii", "heyy" , "hey buddy" ,  "hiii bot", "bot","greeting", "sup", "what's up", "hey" , "Hello", "Hi", "Hey", "Good morning", "Good afternoon", "Good evening", "What's up?", "Howdy", "Greetings", "How's it going?", "Hey there", "Yo!")
    ...
    def greet(sentence):
        if any(word.lower() in user_message for word in nltk.word_tokenize(sentence)):
    ...
    while flag:
        ...
        if greeting_response:
        ...
        elif user_message == "thanks" or user_message == "thank you":
            flag = False
        ...
        elif any(word in user_message for word in ["suggestion", "suggestions", "random", "recommendation", "recommend" , "Recommendation", "Proposal", "Advice", "Tip", "Counsel", "Guidance", "Hint", "Idea", "Suggestion", "Input" , "suggest"]):
            ...
            if recommended_places:
                ...
                st.write("BOT: No recommendations available at the moment.")
        elif "place" in user_message and any(city in user_message for city in data['City'].values):
            # Extract city name from user input
            words = nltk.word_tokenize(user_message)
            cities = [word for word in words if word in data['City'].values]
            for city in cities:
                display_places(city)
        elif any(word in user_message.lower() for word in ["book", "booking", "reservation", "reserve"]):
        ...
        elif any(word in user_message.lower() for word in ["weather", "climate", "atmosphere", "meteorological", "elements", "forecast", "conditions", "temperature", "data", "meteorology"]):
            words = nltk.word_tokenize(user_message)
            for word in words:
        ...
        elif is_location_present(user_message):
            words = nltk.word_tokenize(user_message)
            location = None
            for word in words:
                if is_location_present(word):
            ...
            if location:
                ...
                if weather_description:
                    ...
                    st.write("BOT: Failed to fetch weather data.")
                response_sentences = get_similar_sentences(user_message)
                if response_sentences:
                    for sentence in response_sentences:
                            st.write("- " + sentence)
                else:
                    pass
        elif any(word.lower() in user_message for word in nltk.word_tokenize(user_message)):
